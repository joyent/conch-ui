          ...
          <div class="datatable-header">
            ...
            <div class="select-with-label">
              ...
              <div class="select">
                ...
                >
                  <option value="all">All</option>
                  <option :value="build" v-for="build in builds" :key="build">
                    {{ build }}
                  </option>
                </select>
              </div>
...
export default {
  ...
  methods: {
    ...
    changeFilter(event, filter) {
      const eventValue = (event && event.target && event.target.value) || 'all';
  ...
  computed: {
    ...
    filteredRacks() {
      ...
      if (this.roleFilter !== 'all') {
        racks = racks.filter(
          rack =>
            rack.rack_role_name.toLowerCase() === this.roleFilter.toLowerCase()
        );
      }
      ...
      if (this.buildFilter !== 'all') {
        racks = racks.filter(
          rack =>
            rack.build_name.toLowerCase() === this.buildFilter.toLowerCase()
        );
      }

      if (this.phaseFilter !== 'all') {
        racks = racks.filter(
          rack => rack.phase.toLowerCase() === this.phaseFilter.toLowerCase()
        );
      }
