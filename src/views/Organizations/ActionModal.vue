      ...
      <div class="modal-card">
        ...
        <div class="modal-card-body">
          <table class="table is-fullwidth">
            <tbody>
              <tr
                v-if="itemList.length || (itemList.length === 0 && searchText)"
              >
              ...
              <template v-if="itemList.length">
                <tr
                  class="row"
                  :class="{
                    'is-adding': action === 'add' && isModified(item.id),
                    'is-removing': action === 'remove' && isModified(item.id),
                  }"
                ...
                >
                  <template v-if="isModified(item.id)">
                    ...
                    <td class="organization-role-select">
                      <div class="select role" v-if="action === 'add'">
                        <select @change="updateRole(item.id, $event)">
                          <option value="admin">
                            Admin
                          </option>
                          <option value="rw">
                    ...
                    <td class="action">
                      <i
                        ...
                        v-if="showRemoveIcon !== item.name && action === 'add'"
                      ...
                      <i
                        ...
                        v-if="showAddIcon !== item.name && action === 'remove'"
              ...
              <template v-else-if="!itemList.length && !searchText">
      ...
      if (this.action === 'add') {
        if (this.itemType === 'members') {
          for (let i = 0; i < dataLength; i++) {
            const member = data[i];
            await addUserToOrganization(organizationId, member.role, member.id);
          }
        ...
        if (this.itemType === 'members') {
          for (let i = 0; i < dataLength; i++) {
            const member = data[i];
            await removeUserFromOrganization(organizationId, member.id);
          }
          ...
          for (let i = 0; i < dataLength; i++) {
            const build = data[i];
            await Builds.removeOrganizationFromBuild(build.id, organizationId);
          }
