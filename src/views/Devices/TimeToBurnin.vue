<template>
  <div class="time-to-burnin">
    ...
    <p class="is-size-4" v-else-if="activeDeviceDetails.last_seen == null">
      Device has not reported
    </p>
    <p
      class="is-size-4"
      v-else-if="activeDeviceSettings.firmware !== 'current'"
    >
      Burn-in not started
    </p>
  ...
  computed: {
    ...
    sinceLastReboot() {
      return this.uptimeSince
        ? moment().diff(moment(this.uptimeSince), 'seconds')
        : moment().diff(moment(this.activeDeviceDetails.last_seen), 'seconds');
    },
    percentage() {
    ...
    time() {
      return (
        this.maxBurnin -
        (this.rebootCount * this.burninStageTime + this.sinceLastReboot)
      );
    },
    rebootCount() {
