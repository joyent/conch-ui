            ...
            <tbody>
              ...
              >
                ...
                <td class="permissions">
                  <span class="is-capitalized" v-if="user.role == 'admin'">
                    All
                  </span>
                  <span class="is-capitalized" v-else-if="user.role === 'ro'">
                    Read Only
                  </span>
                  <span class="is-capitalized" v-else-if="user.role === 'rw'">
                    Read &#47; Write
                  </span>
                </td>
                <td class="remove-item has-text-right">
                  <i
                    ...
                    v-if="user.role !== 'admin' || adminUsersCount > 1"
...
export default {
  ...
  methods: {
    ...
    changeFilter(event) {
      this.$router.push({
        ...
        query: {
          role: (event && event.target && event.target.value) || 'all',
        },
      });
    },
    closeModal() {
  ...
  computed: {
    ...
    adminUsersCount() {
      if (this.buildHasUsers) {
        return this.currentBuildUsers.filter(user => user.role === 'admin')
          .length;
      }
  ...
  created() {
    ...
    EventBus.$on(['close-modal:add-item', 'close-modal:remove-item'], () => {
      this.closeModal();
    });
