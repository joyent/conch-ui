...
describe('authentication API', () => {
    const errorCredentialsRequired = { error: '"user" and "password" are both required' };
    ...
    describe('login', () => {
        ...
        test('should return an error if user is empty', () => {
            ...
            return expect(authentication.login(data)).rejects.toMatchObject(errorCredentialsRequired);
        });
        ...
        test('should return an error if password is not at least 5 characters', () => {
            ...
            return expect(authentication.login(data)).rejects.toMatchObject(errorPwdLength);
        });
        ...
        test('should return an error if password does not have a match in the db', () => {
            ...
            return expect(authentication.login(data)).rejects.toMatchObject(errorUnauthorized);
        });
        ...
        test('should return jwt_token upon successful login', () => {
            ...
            return expect(authentication.login(data)).resolves.toMatchObject({ jwt_token: true });
        });
    });
    ...
    describe('isLoggedIn', () => {
        ...
        test('should return rejected promise if user is not logged in', () => {
            expect.assertions(1);
            return expect(authentication.isLoggedIn(null)).rejects.toEqual(false);
        });
    });
});
