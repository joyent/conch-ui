            ...
            <div class="column is-4 step-details">
              <div class="step-content">
                ...
                <template v-if="activeStep === 1">
                  ...
                  <br />
                  <p>
                    Give the new organization a name and description.
                  </p>
                </template>
                <template v-else-if="activeStep === 2">
                  ...
                  >
                    Every organization must have at least one user with admin
                    privileges.
                  </p>
                  <br />
                </template>
                ...
                <template v-else>
                  ...
                  <br />
                  <p>
                    Review the details of the new organization and make any
                    changes necessary.
                  </p>
                  <br />
                </template>
            ...
            <div class="column organization-details">
              ...
              <div class="modal-body">
                <transition name="fade">
                    ...
                    <div class="item-table">
                      <table class="table is-fullwidth">
                        <tbody>
                          ...
                          <template v-if="filteredItems('members').length > 0">
                            <tr
                              class="row item"
                              :class="{
                              ...
                              v-for="user in filteredItems('members')"
                              :key="user.id"
                            >
                              <template
                              ...
                              >
                                ...
                                <td class="role-select">
                                  <div class="select role">
                                    <select
                                      @change="
                                        updateRole('member', user.name, $event)
                                      "
                                    >
                                      <option
                                        value="admin"
                                ...
                                <td class="action">
                                  ...
                                  <i
                                    ...
                                    v-if="showRemoveIcon === user.name"
                                    @click="removeItem(user.name, 'member')"
                    ...
                    <div class="item-table">
                      <table class="table is-fullwidth">
                        <tbody>
                          ...
                          <template v-if="filteredItems('builds').length > 0">
                            <tr
                              class="row item"
                              :class="{
                                'is-selected': isItemSelected(
                              ...
                              v-for="build in filteredItems('builds')"
                              :key="build.id"
                            >
                              <template
                                v-if="isItemSelected(build.name, 'builds')"
                              >
                                <td class="item-name">
                                  <span class="name has-text-grey-light">
                                    {{ build.name }}
                                ...
                                <td class="role-select">
                                  <div class="select role">
                                    <select
                                      @change="
                                        updateRole('build', build.name, $event)
                                      "
                                    >
                                      <option
                                        value="admin"
                                        :selected="build.role === 'admin'"
                                      >
                                        Admin
                                      </option>
                                      <option
                                        value="rw"
                                        :selected="build.role === 'rw'"
                                      >
                                        Read &#47; Write
                                      </option>
                                      <option
                                        value="ro"
                                        :selected="build.role === 'ro'"
                                      >
                                        Read Only
                                      </option>
                                    </select>
                                ...
                                <td class="action">
                                  <i
                                    class="material-icons has-text-success item-added"
                                    v-if="showRemoveIcon !== build.name"
                                    @mouseover="showRemoveIcon = build.name"
                                  >
                                    check
                                  </i>
                                  <i
                                    class="material-icons has-text-danger remove-item"
                                    v-if="showRemoveIcon === build.name"
                                    @click="removeItem(build.name, 'build')"
                                    @mouseleave="showRemoveIcon = ''"
                                  >
                                    close
                                  </i>
                          ...
                          <tr
                            class="row item"
                            v-else-if="
                              filteredItems('builds').length === 0 && searchText
                            "
                          >
                            <td class="has-text-centered no-results">
                              No search results
                  ...
                  <div class="step-body review" v-else-if="activeStep === 4">
                    ...
                    <div class="review-item members">
                      ...
                      >
                        <div class="item-content-row empty">
                          No members selected
                        </div>
                      </div>
                    </div>
                    <div class="review-item builds">
                      ...
                      <div
                        class="review-item-content"
                        v-if="isExpandedBuilds && selectedBuilds.length"
                      >
                        <div
                          class="item-content-row"
                      ...
                      <div
                        class="review-item-content"
                        v-else-if="isExpandedBuilds && !selectedBuilds.length"
                      >
                        <div class="item-content-row empty">
                          No builds selected
                        </div>
              ...
              <div class="modal-footer">
                ...
                >
                  ...
                  <a class="button next is-info" @click="tryNextStep()">
                    Next
                  </a>
                </div>
                <a
  ...
  methods: {
    ...
    async createOrganization() {
      ...
      for (let i = 0; i < this.selectedBuilds.length; i++) {
        ...
        await addOrganizationToBuild(build.id, this.organizationId, build.role);
      }
    ...
    isItemSelected(itemName, itemType) {
      ...
      if (itemType === 'members' && selectedMembers.length) {
        return selectedMembers.map(user => user.name).indexOf(itemName) !== -1
          ? true
          : false;
      } else if (itemType === 'builds' && selectedBuilds.length) {
        return selectedBuilds.map(build => build.name).indexOf(itemName) !== -1
          ? true
          : false;
      }
    ...
    removeItem(itemName, itemType) {
      ...
      if (itemType === 'member') {
        ...
        index = this.selectedBuilds.map(build => build.name).indexOf(itemName);
