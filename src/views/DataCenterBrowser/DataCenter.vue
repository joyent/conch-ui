<template>
  <div class="data-center">
    ...
    <div class="data-center-details has-text-grey">
      ...
      <span style="margin-right: 20px;">{{ currentDataCenter.location }}</span>
      ...
      <ul>
        ...
        >
          <router-link
            ...
            tag="a"
          >
            <span v-if="currentDataCenter && currentDataCenter.region">
              {{ `Rooms (${currentDataCenter && currentDataCenter.region})` }}
            </span>
        ...
        >
          <router-link
            ...
            tag="a"
          >
            <span v-if="currentDataCenterRoom && currentDataCenterRoom.alias">
        ...
        >
          <router-link
            ...
            tag="a"
          >
            <span v-if="currentDataCenterRack && currentDataCenterRack.name">
    ...
    async fetchData() {
      ...
      if (routeQuery) {
        if (routeQuery.room && isEmpty(this.currentDataCenterRoom)) {
          const roomsResponse = await getDataCenterRooms(routeParams.id);
        ...
        if (routeQuery.rack && isEmpty(this.currentDataCenterRack)) {
          const racksResponse = await getDataCenterRoomRacks(routeQuery.room);
