    ...
    <div class="columns">
      <div class="column">
        <div class="datatable-header">
          <p class="datatable-header-title is-size-5 has-text-white">
            {{
              `Builds (${(organization.builds && organization.builds.length) ||
                0})`
            }}
          </p>
          <div
              ...
              >
                ...
                <td class="row-action-button">
                  <a
                    v-if="hasPermissions"
                    class="button-delete"
                    @click.stop="showRemoveItemModal(build, 'build')"
                  >
                    <span class="icon">
                      <i class="material-icons">delete</i>
                    </span>
      ...
      <div class="column">
        <div class="datatable-header">
          <p class="datatable-header-title is-size-5 has-text-white">
            {{
              `Members (${(organization.users && organization.users.length) ||
                0})`
            }}
          </p>
          <div
            ...
            <div class="dropdown-menu">
              <div class="dropdown-content">
                ...
                <a
                  class="dropdown-item remove"
                  v-if="
                    hasPermissions && !editMembers && organizationHasMembers
                  "
                  @click="openActionModal('remove', 'members')"
                >
                  <i class="material-icons">delete</i>
                  <p>Remove Members</p>
                </a>
            ...
            <tbody>
              ...
              >
                ...
                <template v-if="editMembers">
                  <td>
                    <div class="select">
                      <select @change="updateRole(member, $event)">
                        <option
                  ...
                  <td>
                    <div class="select" v-if="member.role !== 'admin'">
                      <select
                        v-model="member.role"
                        @change="updateRegularUserPermissions(member, $event)"
                      >
                        <option value="ro">
                          Read Only
                        </option>
                ...
                <td class="row-action-button">
                  <a
                    class="button-delete"
                    @click.stop="showRemoveItemModal(member, 'member')"
                    v-if="
                      hasPermissions &&
                        (member.role !== 'admin' || adminMembersCount > 1)
                    "
                  >
                    <i class="material-icons">delete</i>
                  </a>
        ...
        <div class="datatable-footer" v-if="hasPermissions && !editMembers">
          <a
        ...
        >
          <a
            ...
            @click="saveChanges()"
            :disabled="modifiedMembers && modifiedMembers.length === 0"
          >
...
export default {
  ...
  methods: {
    ...
    async removeItemFromOrganization() {
      ...
      if (itemType === 'member') {
        await Organizations.removeUserFromOrganization(organizationId, itemId);
      } else if (itemType === 'build') {
        await Builds.removeOrganizationFromBuild(itemId, organizationId);
      }
    ...
    updateRegularUserPermissions(member, event) {
      ...
      const newPermissions =
        event && event.target && event.target.value ? event.target.value : 'ro';
