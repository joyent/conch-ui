<template>
  <div class="modal edit-layout-modal" :class="{ 'is-active': isActive }">
    ...
    <div class="modal-card">
      ...
      <article
        v-if="invalidSerialNumber || duplicateSerialNumber || duplicateAssetTag"
        ...
        <table class="table is-fullwidth is-hoverable is-marginless">
          ...
          <tbody>
            <tr
              ...
              :key="assignment.slot"
              :class="{
                'is-editing': isEditingAssignment(assignment.slot),
                'has-error': assignmentHasError(assignment.slot),
              }"
            >
              ...
              <td>
                <a
                  ...
                  @click="
                    navigateToHardwareProduct(assignment.hardware_product_name)
                  "
                >
                  {{ assignment.hardware_product_name }}
                </a>
              </td>
              <template v-if="isEditingAssignment(assignment.slot)">
                <td class="serial-number">
                  <div
                    class="control has-icons-right"
                    :class="{
                      'tooltip is-tooltip-danger':
                        hasDuplicateSerialNumber(assignment.slot) ||
                        hasInvalidSerialNumber(assignment.slot),
                    }"
                    :data-tooltip="getTooltip(assignment.slot)"
                  >
                    <input
                      type="text"
                      ...
                      v-model.trim="assignment.serial_number"
                ...
                <td class="asset-tag">
                  ...
                  >
                    <input
                      ...
                      :class="{
                        'is-danger': hasDuplicateAssetTag(assignment.slot),
                      }"
                    ...
                    <span
                      class="icon is-small is-right"
                      v-if="hasDuplicateAssetTag(assignment.slot)"
                    >
                      <i class="material-icons has-text-danger">
                        error
                      </i>
              ...
              <template v-else>
                <td class="serial-number" v-if="assignment.serial_number">
                  {{ assignment.serial_number }}
                </td>
                <td class="serial-number" v-else>
                  None
                </td>
                <td class="asset-tag" v-if="assignment.asset_tag">
              ...
              <td class="edit-action">
                <a
                  ...
                  @click="cancelEditAssignment(assignment.slot)"
                >
                  <span>Cancel</span>
                </a>
                <a
      ...
      <footer class="modal-card-foot">
        ...
        <a class="button is-fullwidth close-modal" @click="closeModal()" v-else>
          Close
        </a>
      </footer>
    </div>
  ...
  methods: {
    ...
    cancelEditAssignment(slot) {
      ...
      for (let i = 0; i < this.assignments.length; i++) {
        if (this.assignments[i].slot === slot) {
          ...
          this.assignments[i].serial_number = assignment.original_serial_number;
          this.assignments[i].asset_tag = assignment.original_asset_tag;
    ...
    validateInput() {
      ...
      for (let i = 0; i < modifiedAssignments.length; i++) {
        ...
        for (let i = 0; i < this.devices.length; i++) {
          ...
          if (
            ...
            const duplicatedAssetTagModified = modifiedAssignments.some(
              modifiedAssignment =>
                assetTag === modifiedAssignment.original_asset_tag &&
          ...
          if (
            ...
            const duplicatedSerialNumberModified = modifiedAssignments.some(
              modifiedAssignment =>
                serialNumber === modifiedAssignment.original_serial_number &&
        ...
        if (duplicateAssetTag || duplicateSerialNumber || invalidSerialNumber) {
