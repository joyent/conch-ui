    ...
    >
      ...
      <div class="modal-card">
        ...
        <section class="modal-card-body">
          <p>
            {{ selectedUsers.length }} users have been successfully added.
          </p>
          <br />
...
export default {
  ...
  methods: {
    ...
    isUserSelected(userId) {
      return this.selectedUsers.map(user => user.id).indexOf(userId) !== -1;
    },
    removeMember(userId) {
      const index = this.selectedUsers.map(user => user.id).indexOf(userId);
