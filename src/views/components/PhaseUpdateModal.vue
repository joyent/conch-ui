  ...
  <div>
    <BaseModal class="phase-update-modal" v-if="!updateSuccess">
      ...
      <template v-slot:body>
        <div class="subtitle has-text-centered" style="margin-bottom: 10px;">
          <span class="device-serial-number" v-if="item === 'device'">
            Device:
            <strong class="has-text-white">{{ itemData.serial_number }}</strong>
          </span>
          <span class="rack-name" v-else-if="item === 'rack'">
            Rack:
            <strong class="has-text-white">{{ itemData.name }}</strong>
          </span>
        </div>
    ...
    <BaseModal v-else-if="!updatingPhase && updateSuccess">
      ...
      <template v-slot:body>
        <p class="subtitle">
          <span>
            ...
            <strong class="has-text-white" v-else-if="item === 'rack'">
              {{ itemData.name }}
            </strong>
            has been set to
      ...
      <template v-slot:footer>
        <a class="button confirm is-success is-fullwidth" @click="closeModal()">
          <span>Great!</span>
          <i class="fas fa-lg fa-long-arrow-alt-right"></i>
        </a>
      </template>
