<template>
  <div class="user-action-modal">
    ...
    <BaseModal v-else-if="!success && deactivateConfirmed">
      ...
      <template v-slot:footer>
        <a class="button confirm is-success is-fullwidth" @click="confirm()">
    ...
    <BaseModal v-else>
      ...
      <template v-slot:body>
        ...
        <p class="subtitle" v-else>
          <strong class="has-text-white">{{ user.name }}</strong> has been
          successfully <strong class="has-text-white">{{ action }}d</strong>.
        </p>
      </template>
      <template v-slot:footer>
